...
\begin{center}
\begin{longtable}{ p{.2\textwidth}  p{.4\textwidth}  p{.3\textwidth} } 
\textbf{Stage} & \textbf{Stats} & \textbf{Total time} \\\toprule
\multirow{2}{80pt}{\textbf{Descriptors computation}} & Minimum time for track: 00m 15s & \multirow{0}{80pt}{04h 32m 25s} \\ \cmidrule(lr){2-2}
& Maximum time for track: 00m 52s &  \\ \cmidrule(lr){2-2}
& Average time for track: 00m 28s &  \\ \midrule
\multirow{2}{80pt}{\textbf{FastMap computation}} & Choosing pivots: 16m 43s & 47m 12s \\ \cmidrule(lr){2-2}
